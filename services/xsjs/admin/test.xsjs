...
function getTokenSession(reqBody){
	...
	try {
		...
		var fnUser = conn.loadProcedure('CART_REQUEST_TOOL',
				'mktgcartrequesttool.db.procedures::GET_USER_BY_USERNAME');
	...
	try {
		...
		var spUserToken = conn2
				.loadProcedure('CART_REQUEST_TOOL',
						'mktgcartrequesttool.db.procedures::GET_USER_SESSION_TOKEN_BY_USER_ID');
...
function getPasswordHash(userPassword) {
	...
	try {
		...
		var spHash256 = conn.loadProcedure('CART_REQUEST_TOOL',
				'mktgcartrequesttool.db.procedures::GET_HASH_SHA256');
...
function insertUserRole(){
	...
	try {
		...
		var fn = conn.loadProcedure('CART_REQUEST_TOOL',
				'mktgcartrequesttool.db.procedures::INS_USER_ROLE');
...
function updateUserRole(){
	...
	try {
		...
		var fn = conn.loadProcedure('CART_REQUEST_TOOL',
				'mktgcartrequesttool.db.procedures::UPD_USER_ROLE_BY_USER_ID');
