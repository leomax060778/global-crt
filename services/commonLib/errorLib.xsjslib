function getErrors() {
    ...
    function parseDetails(details) {
        ...
        if (typeof details === "object") {
            result = JSON.stringify(details).replace("{","\\{").replace("[","\\[");
        }
        return result;
    }
    ...
    Errors.BadRequest = function (message, stack, details) {
        var parsedDetails = parseDetails(details);
        var e = {};
        e.name = "Bad Request";
        e.message = message || "Bad Request";
        e.code = 400;
        e.stack = stack || commonStack;
        e.details = parsedDetails || "without details";
        e.toString = function () {
    ...
    Errors.Unauthorized = function (message, stack, details) {
        ...
        e.details = parsedDetails || "without details";
        e.toString = function () {
            return "name:" + e.name + " -message:" + e.message + " -code:" + e.code + " -stack:" + e.stack + " -details:" + e.details;
        };
        return e;
    };
